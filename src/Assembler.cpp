...
{   
    ...
    if (argc == 1) {
        fprintf(stderr, "WARNING: A file wasn't given, so default asmfile will be used.\n");
        Assemble(DEFAULT_ASMFILE, &ASM);
    } else {
        Assemble(argv[1], &ASM);
    }
    
    Outstream(DEFAULT_OUTFILE, &ASM); 
    ASMDtor(&ASM);
...
{   
    ...
    while (asm_count++ < 2) {
        ...
        if ((ReadFile = fopen(asm_file, "r")) == 0) {
            ...
            ReadFile = fopen(DEFAULT_ASMFILE, "r");
        ...
        while(ASM->ip < MAX_CMD) { 
            ...
            if (strcmp(cmd, _ADD_) == 0) {
                ASM->code[ASM->ip++] = ADD;
                continue;
            }

            if (strcmp(cmd, _SUB_) == 0) {
                ASM->code[ASM->ip++] = SUB;
                continue;
            }

            if (strcmp(cmd, _MUL_) == 0) {
                ASM->code[ASM->ip++] = MUL;
                continue;
            }

            if (strcmp(cmd, _SQRT_) == 0) {
                ASM->code[ASM->ip++] = SQRT;
                continue;
            }

            if (strcmp(cmd, _DIVIDE_) == 0) {
                ASM->code[ASM->ip++] = DIV;
                continue;
            } 

            if (strcmp(cmd, _OUTPUT_) == 0) {
                ASM->code[ASM->ip++] = OUT;
                continue;
            }

            if (strcmp(cmd, _INPUT_) == 0) {
                ASM->code[ASM->ip++] = IN;
                continue;
            }
            
            if (strcmp(cmd, _JUMP_) == 0) {
                int arg = -1;
            ...
            if (strcmp(cmd, _HLT_) == 0) {
                ASM->code[ASM->ip++] = HLT;
                continue;
            }

            if (strcmp(cmd, _CALL_) == 0) {
                int arg = -1;
                ...
                ASM->code[ASM->ip++] = CALL;
                ASM->code[ASM->ip++] = arg;
                continue;
            }

            if (strcmp(cmd, _RETURN_) == 0) {
                ASM->code[ASM->ip++] = RET;
                continue;
            }
...
int Outstream(const char* filename, ASSEMBLER *ASM)
{
    ...
    FILE *out_file = fopen(filename, "wb"); assert(out_file);
...
size_t Labelfind(ASSEMBLER *ASM, const char *str) 
{
    ...
    for (size_t i = 0; i < MARKER_AMOUNT; i++) {
        if (strcmp(ASM->labels[i].name, str) == 0) {
            ...
            ON_DEBUG(CYAN_PRINT_LOG(stderr, "i = %lu\n", i));
...
int ASMDump(ASSEMBLER *ASM) 
{
    ...
    WHITE_PRINT_LOG(stderr, "Labels:\n");
    
    for (size_t i = 0; i < MARKER_AMOUNT; i++)
        fprintf(stderr, "%lu: index = %d, name = %s\n", i, (ASM->labels[i]).index, ASM->labels[i].name);
    putchar('\n');
